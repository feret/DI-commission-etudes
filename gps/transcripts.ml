...
type cours =
  {
    ...
    code_cours_helisa: string option;
    code_cours_gps: string option;
  ...
  store_gen
    ...
    let is_stage cours =
      begin
        ...
        [Some "UNEXPA-39";Some "UNEXPA-34";Some "UNEXPA-08";Some "UNEXPA-37"; Some "UNEXPA-38" ; Some "UNINF1-042" ; Some "UNEXPA-07"]
      end
      || 
      begin 
    ...
    if b then 
      ...
      List.fold_left 
        (fun (state, course_list_ok, course_list_all, map) a -> 
            let state, course_list_all =  Reglements_pedagogiques_tools.CourseDMap.add a course_list_all state in 
...
let add_mean is_m2 state key compensation unvalidated course course_list_ok course_list_all year map dens =
  ...
  | None,_, None   -> add_mean_potential is_m2 state key course course_list_ok 
  course_list_all year map dens 
  | None,_, Some note ->
    if note = Public_data.En_cours then add_mean_potential is_m2 state key course course_list_ok 
  course_list_all year map dens  else 
      match
        Notes.valide note
      with
      ...
      (fun
        ...
        let state = if is_focus then 
            Remanent_state.warn __POS__ 
              (Format.sprintf "COURS %s %s %s %s %f %s %s %s %s" lastname codecours 
              ...
              (Public_data.string_of_dpt dpt)
              (Tools.unsome_string cours.diplome)
...
in 
        ...
        in
          let state = 
    if focus then 
      Remanent_state.warn __POS__ (Format.sprintf "ADD EXTRA COURSE1 %s %s %f %f %s %s %s %s" lastname (Tools.unsome_string libelle)  (match ects with None -> 0. | Some a -> a) (match cours.ects with None -> 0. | Some a -> a) (Tools.unsome_string string)
              (Public_data.string_of_dpt dpt)  (Tools.unsome_string cours.diplome)
              ( Public_data.string_of_dpt_opt cours.diplome_dpt)) Exit state 
    else state 
  in   
        let cours = 
       ...
       let state = 
    if focus then 
      Remanent_state.warn __POS__ (Format.sprintf "ADD EXTRA COURSE2 %s %s %f %f %s %s %s %s" lastname (Tools.unsome_string libelle)  (match ects with None -> 0. | Some a -> a) (match cours.ects with None -> 0. | Some a -> a)  (Tools.unsome_string string)
              (Public_data.string_of_dpt dpt) (Tools.unsome_string cours.diplome)
              ( Public_data.string_of_dpt_opt cours.diplome_dpt) ) Exit state 
    else state 
  in   
  ...
  (match l_en with None -> "" | Some l -> l);
    ...
    let () =
      Remanent_state.fprintf
        ...
        "\\npnoround%%\n\ "
    in
    state,moyenne_value,Format.sprintf "\\thegradedects%s"
        key, cours_list, stage_list
...
let code_map = fill ects_4 4. code_map 
          ...
          let elt =
            {
              ...
              code_cours =
                ...
                code_cours_gps = course.Public_data.pe_code_gps ; 
                code_cours_helisa = Some course.Public_data.pe_code_helisa ; 
              responsable = None ;
            ...
            let elt =
              {
                ...
                code_cours ;
                code_cours_helisa = Some codehelisa ; 
                code_cours_gps = course.Public_data.pegasus_codegps; 
                responsable = None ;
  ...
  | Some out ->
    ...
    let state,mean,dens,natt, is_l3, cours_list_ok, cours_list_all, stage_list =
      List.fold_left
        ...
        (state,mean_init,dens_init,n_att_init, false,cours_list_init,Reglements_pedagogiques_tools.CourseDMap.empty,stage_list_init)
        l
    in
    let _ = natt in
    ...
    then 
        if lmath && linfo situation then 
          ...
          [(Some Public_data.L3, Some Public_data.DMA), Reglements_pedagogiques.licence_maths_mathsphys ; 
           (Some Public_data.L3, Some Public_data.PHYS), Reglements_pedagogiques.licence_phys_mathsphys] 
        else if lmath && lbio situation then   
          [(Some Public_data.L3, Some Public_data.DMA), Reglements_pedagogiques.licence_maths_mathsbio ; 
           (Some Public_data.L3, Some Public_data.IBENS), Reglements_pedagogiques.licence_bio_mathsbio] 
          else    if is_l3 
                ...
                if year > current_year then
                 ...
                 then
                   ...
                   begin
                     ...
                     let state, mean, ects_qui_comptent, cours_list, stage_list
