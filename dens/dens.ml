...
let actd = "ACTD","UNEXP"
let info = Public_data.prefix_code_of_dpt Public_data.DI,"UNINF"
let dma = Public_data.prefix_code_of_dpt Public_data.DMA,"UNDMA"
let bio = Public_data.prefix_code_of_dpt Public_data.IBENS,"UNBIO"
let phys = Public_data.prefix_code_of_dpt Public_data.PHYS,"UNPHY"
let chimie = Public_data.prefix_code_of_dpt Public_data.CHIMIE,"UNCHI"
let gsc = Public_data.prefix_code_of_dpt Public_data.GEOG,"UNGSC"
let phil = "PHIL","UNPHI"
let dec = Public_data.prefix_code_of_dpt Public_data.DEC,"UNDEC"
let arts = "ARTS","UNART"
let dsa = Public_data.prefix_code_of_dpt Public_data.DSA,"UNDSA"
let dss = Public_data.prefix_code_of_dpt Public_data.DSS,"UNDSS"
let eco = Public_data.prefix_code_of_dpt Public_data.ECO,"UNECO"
let lila = Public_data.prefix_code_of_dpt Public_data.LILA,"UNLIT"
let geog = Public_data.prefix_code_of_dpt Public_data.GEOG,"UNGEO"
let hist = Public_data.prefix_code_of_dpt Public_data.HIST,"UNHIS"
let ceres = "CERES","UNCER"
let ciens = Public_data.prefix_code_of_dpt Public_data.CIENS,"UNCIE"
let ecla = Public_data.prefix_code_of_dpt Public_data.ECLA,"UNECL"
let vetu = "VETU","UNEXP"
let dg = "DG",""
let ens = "DENS",""
let dri = Public_data.prefix_code_of_dpt Public_data.DRI,""
let xt = "XT",""
    ...
    then
      ...
      | Ecla ->
        let state = 
          let state,b = Remanent_state.is_focus ~firstname ~lastname state in 
           if b then 
                Remanent_state.warn __POS__ (Format.sprintf "%s ECLA %s %f" firstname codegps course.Public_data.supplement_ects) Exit state 
           else state 
        in 
      ...
      begin 
        let state = 
          let state,b = Remanent_state.is_focus ~firstname ~lastname state in 
          if b then 
                Remanent_state.warn __POS__ (Format.sprintf "ACTIVITE %s %s %f" lastname codegps course.Public_data.supplement_ects) Exit state 
              else state 
            in 
        ...
        with 
          | state, Some Public_data.Recherche -> 
            ...
            let state = 
              if b then 
                Remanent_state.warn __POS__ (Format.sprintf "EXP RECHERCHE %s %s %f" lastname codegps course.Public_data.supplement_ects) Exit state 
              else state 
            in 
          ...
          | state, Some Public_data.Internationale -> 
            let state = 
              let state,b = Remanent_state.is_focus ~firstname ~lastname state in 
              if b then 
                Remanent_state.warn __POS__ (Format.sprintf "EXP INTERNATIONALE %s %s %f" lastname codegps course.Public_data.supplement_ects) Exit state 
              else state 
            in 
          ...
          | state, Some Public_data.Ouverture -> 
            let state = 
             let state,b = Remanent_state.is_focus ~firstname ~lastname state in   
              if b then 
                Remanent_state.warn __POS__ (Format.sprintf "EXP OUVERTURE %s %s %f" lastname codegps course.Public_data.supplement_ects) Exit state 
              else state 
            in 
              ...
              | state, Some Public_data.Promotion -> 
                let state = 
                   let state,b = Remanent_state.is_focus ~firstname ~lastname state in 
                   if b then 
                      Remanent_state.warn __POS__ (Format.sprintf "EXP PROMOTION %s %s %f" lastname codegps course.Public_data.supplement_ects) Exit state 
                  else state 
                in 
          ...
          | state, Some Public_data.Transdisciplinaire -> 
            let state = 
              ...
              if b then 
                ...
                Remanent_state.warn __POS__ (Format.sprintf "EXP TRANSDISCIPLINAIRE %s %s %f" lastname codegps course.Public_data.supplement_ects) Exit state 
              else state 
            in 
          ...
          | state, (Some Public_data.Hors_Dens | None) -> 
            let state = 
              ...
              if b then 
                Remanent_state.warn __POS__ (Format.sprintf "EXP HORS DENS %s %s %f" lastname codegps course.Public_data.supplement_ects) Exit state 
              else state 
            in 
    ...
    | Humanities | Sciences | Sans_mineure ->
      let state = 
             let state,b = Remanent_state.is_focus ~firstname ~lastname state in 
              ...
              if b then 
                Remanent_state.warn __POS__ (Format.sprintf "OTHER  %s %s %f" lastname codegps course.Public_data.supplement_ects) Exit state 
              else state 
            in 
      ...
      | Missing ->
        let state = 
              ...
              if b then 
                Remanent_state.warn __POS__ (Format.sprintf "EXP MISSING %s %s %f" lastname codegps course.Public_data.supplement_ects) Exit state 
              else state 
            in 
      let dens_cours_a_trier = dens.Public_data.dens_cours_a_trier in
      let list = get dens_cours_a_trier in
