...
let print_preamble state dens =
   ...
   let () = Remanent_state.fprintf state
    ...
    let () = Remanent_state.print_newline state in
    let () = Remanent_state.fprintf state "\\item  d'une formation complémentaire, dispensée ou agréée par l'ENS, répartie sur trois ans. Le volume global de cette formation doit représenter au moins un volume de travail additionnel équivalent à 1/5 (soit 20\\%) de la formation principale." in
    let () = Remanent_state.print_newline state in
    ...
    let () = Remanent_state.print_newline state in
    let () = Remanent_state.fprintf state "{\\noindent}Français : 75 \\%" in
    let () = Remanent_state.print_newline state in
    let () = Remanent_state.fprintf state "{\\noindent}Anglais : 25 \\%" in
    let () = Remanent_state.print_newline state in
