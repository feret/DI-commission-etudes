...
let state =
  ...
  let state, t = Remanent_state.Collector_course_pegasus.get state 
      let () = Pegasus_courses.dump t 
  ...
  let state =
    ...
    if b then
      ...
      let state, t = Remanent_state.Collector_pegasus_stages.get state in
      let () = Pegasus_stages.dump t in
      state
    else state
let state =
...
let state =
  Collect_sorted_internships.get_sorted_internships state
...
let state =
    Dens.get_mineures_candidates state
let state =
    Dens.get_majeures_candidates state
let state, output =
  Cloud_interaction.get_dated_repository state
let state, output_repository =
  match
...
let state, _dpt, signataires =
  ...
  | state, Public_data.CIENS -> state, Public_data.CIENS, []
...
let state =
  StringSet.fold
    (fun promo state ->
       ...
       in
       let state =
         Latex_engine.concat_pdf ~pattern ~exclude:".en.pdf\\|transcript\\|conflicted" ~output
           state
       in
       ...
       in
       let state =
         Latex_engine.concat_pdf ~pattern ~output ~exclude:"transcript\\|conflicted"
           state
       in
